...
Page({
  ...
  // 获取
  getData(orderId) {
    wxRepuest.updateRefundDetails(orderId).then(res => {
      let info = res.data.data
      this.setData({
        info: info,
        refundCause: info.refundCause,
        'formData.remark': info.refundOrder.remark,
        'formData.serviceType': info.refundOrder.serviceType,
        causeid: info.refundOrder.serviceType,
        'formData.reason': info.refundOrder.reason,
        'formData.orderId': info.refundOrder.orderId
      })
      if (this.again) {
        this.setData({
          'formData.id': orderId,
      ...
      this.huiSelect2(info.refundOrder.reason * 1)
  ...
  submit() {
    if (!this.data.formData.serviceType) {
      ...
      wxRepuest.updateRefund(this.data.formData).then(res => {
  ...
  onLoad(e) {
    ...
    e.type = "again" && (this.again = true);
    this.setData({
      orderId: e.orderId,
      ossUrl: app.globalData.siteinfo.imgUrl
    })
    this.getData(e.reOrderId)
